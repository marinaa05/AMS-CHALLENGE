...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = self.up(x1)
        x = torch.cat([x2, x1], dim=1)  # Concatenates the given sequence of tensors in the given dimension
        return self.conv(x)
...
class UNet_base(nn.Module):
    ...
    # Inicializacijska funkcija za uteži v mreži:
    def __init_weight(self):
        # Gremo čez vse podmodule znotraj razreda nn.Module
        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                torch.nn.init.kaiming_normal_(m.weight)  # Inicializacija uteži
            elif isinstance(m, nn.GroupNorm):
                m.weight.data.fill_(1)  # Inicializacija uteži z 1

    def forward(self, x):
        Z = x.size()[2]  # globina
        Y = x.size()[3]  # višina
        X = x.size()[4]  # širina
        diffZ = (16 - Z % 16) % 16  # toliko 0 bomo dodali, da postane Z deljiv s 16
        diffY = (16 - Y % 16) % 16  # 
        diffX = (16 - X % 16) % 16
        # Dodamo ničle v tenzor, da povečamo njegove dimenzije:
        ...
        x = self.up3(x, x2)
        x = self.up4(x, x1)
        # Odstranimo padding, tako da je izhodna dimenzija tenzorja enaka vhodni dimenziji:
